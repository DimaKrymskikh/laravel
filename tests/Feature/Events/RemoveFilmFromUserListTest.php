...
class RemoveFilmFromUserListTest extends TestCase
{
    ...
    public function test_film_can_be_remove_from_user_list(): void
    {
        ...
        $addCityInWeatherList = new RemoveFilmFromUserList($user->id, FilmSeeder::ID_BOILED_DARES);
        
        $broadcastWith = $addCityInWeatherList->broadcastWith();
        // Проверка отправляемого сообщения
        $this->assertEquals($broadcastWith['message'], trans('event_messages.remove_film_from_user_list', ['filmTitle' => $filmTitle]));
