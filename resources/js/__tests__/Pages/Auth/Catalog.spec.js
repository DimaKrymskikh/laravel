...
import Catalog from "@/Pages/Auth/Catalog.vue";
...
describe("@/Pages/Auth/Catalog.vue", () => {
    beforeEach(() => {
    ...
    it("Отрисовка каталога фильмов (залогиненный пользователь)", () => {
        ...
        expect(wrapper.vm.filmsCatalog.page).toBe(films_10_user.current_page);
    ...
    it("Отрисовка каталога фильмов без фильмов (залогиненный пользователь)", () => {
        ...
        // Проверяем, что текущая страница пагинации сохранена в filmsCatalog
        expect(wrapper.vm.filmsCatalog.page).toBe(films_0.current_page);
    ...
    it("Изменение числа фильмов (залогиненный пользователь)", async () => {
        ...
        expect(wrapper.vm.filmsCatalog.page).toBe(5);
        // В начальный момент число фильмов на странице дефолтное
        expect(wrapper.vm.filmsCatalog.perPage).toBe(20);
        ...
        expect(router.get).toHaveBeenCalledWith(wrapper.vm.filmsCatalog.getUrl());
        // В filmsCatalog изменяются текущая страница и число фильмов на странице
        expect(wrapper.vm.filmsCatalog.page).toBe(1);
        expect(wrapper.vm.filmsCatalog.perPage).toBe(50);
    });
    ...
    it("Задание фильтра для фильмов (залогиненный пользователь)", async () => {
        ...
        expect(wrapper.vm.filmsCatalog.page).toBe(5);
        // Поля ввода пустые
        expect(wrapper.vm.filmsCatalog.title).toBe('');
        expect(wrapper.vm.filmsCatalog.description).toBe('');
